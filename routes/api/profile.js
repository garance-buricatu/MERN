...
router.get('/', async (req, res) => {
    try {
        const profiles = await Profile.find().populate('user', ['name', 'avatar']); // populate name and avatar from user collection
        res.json(profiles);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.get('/user/:user_id', async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.params.user_id }).populate('user', ['name', 'avatar']); // findOne by url parameter: user_id
...
router.delete('/', auth, async (req, res) => {
    try {
        ...
        await Post.deleteMany({user: req.user.id }); // delte all instances where user is equal to logged in user
...
router.delete('/education/:edu_id', auth, async (req, res) => {
    try {
        ...
        // Get remove index
        const removeIndex = profile.education.map(item => item.id).indexOf(req.param.edu_id); // get index of experience matching the one that needs to be deleted
...
router.get('/github/:username', (req, res) => {
    try {
        const options = {
            // Note: these fields are obtained by https://github.com/settings/applications/new
